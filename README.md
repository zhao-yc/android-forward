...
这是一个个人侧载使用的 Android MVP，用于把安卓手机收到的系统通知、短信应用通知、来电和未接来电通过 Bark 明文推送到 iPhone。

## 功能特性

- 监听系统通知并转发应用名、标题、正文和时间。
- 短信应用展示系统通知时，会按普通通知转发标题、正文和时间；应用不直接读取短信广播或申请短信权限。
- 监听来电和未接来电，号码不可用时自动降级为未知号码。
- 通过 Bark 官方接口推送到 iPhone。
- Bark Key 使用 Android Keystore 加密保存。
- 可选择已配对蓝牙设备，连接这些设备时自动静默，不发送 Bark。
- 可在最近状态中直接过滤通知来源 APP，并在已过滤应用列表中恢复转发。
- 可开启常驻状态通知，并在通知被清理后尽量自动补发，用来确认应用进程仍在运行。
...
4. 依次授予通知访问、电话状态权限、蓝牙设备权限，并把应用加入省电白名单。
...
- 需要用户手动授予通知使用权、电话状态权限，并把应用加入省电白名单。
- Android 12 及以上需要授予蓝牙设备权限后，才能读取已配对设备和连接状态。
- 常驻状态通知依赖前台服务、通知删除回调和轻量 watchdog；Android 13 及以上还需要通知展示权限，厂商系统强杀后仍需要自启动和省电白名单兜底。
- 项目按个人侧载设计，不按 Google Play 上架合规设计；当前已移除短信直读权限，但通知访问和电话状态仍属于敏感权限。
- iPhone 后台实时到达依赖 Bark/APNs 和网络状态；安卓后台存活依赖厂商系统策略。

## 隐私说明

本项目按个人自用和侧载安装设计。Bark Key 会用 Android Keystore 加密存储；转发日志只保存类型、来源、状态和错误摘要，不保存完整通知正文。开启失败重试时，待重试的推送内容会加密暂存在本机，发送成功或超过重试次数后删除。
